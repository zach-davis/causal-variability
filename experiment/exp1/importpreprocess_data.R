#import and preprocess data pilot2
...
dat <- read.csv('dataexp1.csv')
dat <- dat[-c(seq(1,34)),] #rows 1-34 are other stuff, previews, and pilot runs
dat$participant <- as.factor(seq(1, dim(dat)[1]))
oldcolnames <- names(dat)


#gather data on presentation order of inferences and blocks
ppvars <- c("ProlificID","participant", "Q_TotalDuration", "EndDate", "Q382", "FL_12_DO", "Domain1economicsinferences_DO", "Domain2biologyinferences_DO", "Domain3astronomyinferences_DO", "Domain4meteorologyinferences_DO","Domain5sociologyinferences_DO")
ppdat <- dat[, ppvars]
...
aqvars
aqdat <- dat[,c("ProlificID","participant", "Q_TotalDuration", "EndDate", "Q382", aqvars)]

#gather data on inference level
trialvarnames <- grep("_1$", oldcolnames, value=T) #names of columns with responses
trialdat <- dat[,c("ProlificID", "participant", "Q_TotalDuration", trialvarnames)]
...
#save(datlong, ppdat, aqdat, dat, file="datexp1.RData") #dat 2 graphs and incentivization.
